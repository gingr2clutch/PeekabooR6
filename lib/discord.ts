...
// The joined data a peek embed needs: the peek plus its floor + map names.
...
export function buildPeekEmbed(p: PeekEmbedData): Record<string, unknown> {
  ...
  if (p.poster_url) embed.image = { url: p.poster_url };
...
const EMBED_SELECT =
  "name, slug, poster_url, difficulty, risk, base_success_rate, worked_votes, vote_count, created_at, floors ( name, maps ( name ) )";
...
type EmbedRow = {
  ...
  vote_count: number;
  created_at: string | null;
  floors: { name: string | null; maps: { name: string | null } | null } | null;
};
